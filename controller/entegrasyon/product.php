...
class ControllerEntegrasyonProduct extends Controller
{
    private $error = array();
    private $token_data;
    ...
    protected function getList()
    {
        ...
        /* if($filter_stock_prefix!="*" && !$filter_stock){
        ...
        $data['list_limit'] = $this->config->get('config_limit_admin') + 1;
        ...
        $this->response->setOutput($this->load->view('entegrasyon/product_list', $data));
    }
    ...
    public function check_status()
    {
        ...
        if ($code == 'ty') {
            ...
            //$debug=true;
            $post_data['request_data'] = array('itemcount' => 1, 'page' => 1, 'barcode' => $product_info['model'], 'approved' => true);
            ...
            if (isset($result['result']['content'][0])) {
                ...
                if ($product['rejected']) {
                    ...
                    $rejectReason = $product['rejectReasonDetails'][0]['rejectReason'];
                    $rejectReasonDetail = $product['rejectReasonDetails'][0]['rejectReasonDetail'];
                    $product_status = 'Revize Etmeniz Gerekiyor';

                    $marketplace_data['product_status'] = $product_status;
                    $marketplace_data['message'] = $rejectReason . ':' . $rejectReasonDetail;
                    ...
                    //Ürünü Satışa Açık Konumuna getirebiliriz;

                } else {

                    $marketplace_data['approval_status'] = 0;
                    $product_status = 'Satışta olmayanlar';

                    $marketplace_data['product_status'] = $product_status;
                    $marketplace_data['message'] = 'Ürününüz trendyol tarafından kontol aşamasına alınmıştır..';
                }

            } else {

                $this->entegrasyon->deleteMarketplaceProduct($product_id, $code);
                echo json_encode(array('status' => 0, 'message' => 'Ürün Trendyol mağazanızda bulamadı!'));
                return;
            ...
            if ($result['status']) {
                ...
                if ($result['result']['product_status'] == 'Satışa Hazır') {
                    ...
                    $marketplace_data['approval_status'] = 0;
                    $product_status = $result['result']['product_status'];
                    $marketplace_data['product_status'] = $product_status;
                    $marketplace_data['message'] = implode('-', $result['result']['errors']);
        ...
        echo json_encode(array('status' => 1, 'product_status' => $product_status, 'message' => 'Ürün Durumu:' . $marketplace_data['message']));
    ...
    public function addproduct($product_id = null, $code = null, $bulk = false)
    {
        ...
        */
        ...
        $is_category_varianter = $this->entegrasyon->isCategoryVarianter($category_info[0]);
        ...
        if ($this->config->get($code . '_setting_variant')) {
            ...
            if ($is_varianter_product && ($is_category_varianter || $code=='n11')) {
    ...
    public function send_to_marketplace()
    {
        ...
        if ($this->config->get($code . '_setting_variant')) {
            ...
            if ($this->entegrasyon->isVarianterProduct($product_id) && ($is_category_varianter || $code=='n11')) {
                $product_variants = $this->entegrasyon->getPoductVariants($product_id);
                $market_variants = $this->entegrasyon->getMarketVariant($product_variants, $code, $category_info[0], $product_id, $product_data['model'], HTTPS_CATALOG, array('tax_class_id' => $product_info['tax_class_id'], 'defaults' => $product_data['defaults']));

                if ($code == 'ty' || $code == 'gg' || $code == 'cs' || $code == 'hb') {
    ...
    public function open_for_sale($product_id = null, $code = null, $bulk = false)
    {
        ...
        if ($code == "gg")$this->update($product_id,$code,true);
        ...
        if ($result['status']) {
            ...
            if ($code == 'n11') {
                ...
                $marketplace_data['sale_status'] = 2;
                $marketplace_data['approval_status'] = 1;
            ...
            //print_r($marketplace_data);return;
            $this->entegrasyon->addMarketplaceProduct($product_id, $marketplace_data, $code);
  ...
  */
        ...
        if ($product_info['quantity'] <= 0){
            if (!$this->entegrasyon->isVarianterProduct($product_info['product_id'])){
                $product_info['quantity'] = 0;
                $post_data['request_data']['quantity']=0;
                if ($code == "n11"){
                    $this->close_for_sale($product_id,$code,false);
                    return;
                }
            }
        ...
        //ana ürün başka alandan kodu seçilmiş ise
        if ($this->config->get($code . '_setting_main_product_id')) {
        ...
        if ($result['status']) {
            ...
            if ($product_info['quantity'] && $product_info['sale_price'] && $marketplace_data['approval_status']) {
...
//print_r( $data['s_attr']);
    ...
    public function send_bulk()
    {
        ...
        if ($this->config->get('easy_setting_invisible_image_for_bulk')) {
            $data['is_hidden_image'] = "hidden";
...
*/
            ...
            $data['product_list'] = $this->model_entegrasyon_product->getProducts($filter_data, "for_bulk");
            /*  }else{
            ...
            $data['total_close'] = $product_total;
            ...
            $product_list = explode(',', $this->request->get['product_list']);

            $data['product_list'] = isset($this->request->get['product_list']) ? $this->request->get['product_list'] : '';

            $data['total'] = substr_count($data['product_list'], ",") + 1;

            $data['total_active'] = $data['total'];
            $data['total_passive'] = $data['total'];
            $data['total_close'] = $data['total'];
            ...
            $data['total_active'] = $product_list ? count($product_list) : 0;
    ...
    public function bulk_action_progress()
    {
        ...
        if (isset($this->request->get['bypass'])) {
            $bypass = $this->request->get['bypass'];
        } else {
            $bypass = 0;
        }
        ...
        if ($this->request->get['list_type'] == "all_filter") {
            if (isset($this->request->get['filter_category'])) {
                $filter_category = $this->request->get['filter_category'];
            } else {
                $filter_category = '';
        ...
        if ($list_type == 'selected') {
            ...
            $data['product_list'] = $this->request->get['product_list'];
            $json['total'] = count($product_list);


        }else if($is_filter){

            $product_list = $this->model_entegrasyon_product->getProducts($filter_data);
            ...
            $data['product_list'] = '';
            $json['total'] = count($this->model_entegrasyon_product->getProducts($filter_data));
        ...
        if($list_type == 'selected') {
            ...
            $product_id =  $product_list[$page - 1];

        }else {

            if ($action == 'open_for_sale' || $action == 'close_for_sale' || $action == 'addproduct') {

                if($is_filter){

                    $product_id = $product_list[$bypass]['product_id'];
        ...
        // $result=array('status'=>true,'message'=>$product_id);
        ...
        if (isset($product_info['name'])) {
            $json['message'] = $page . '-' .$product_id.'-'. $product_info['name'] . ' - ' . $result['message'];
        } else {
            $json['message'] = $page . '-' .$product_id.'-'. $result['message'];

        }
        //toplu işlem bilgi kısmı ürün adı -1 bul-..
        $total = $is_filter ? $total_for_progress : count($product_list);
        ...
        if ($is_filter) {
            ...
            $url = 'index.php?route=entegrasyon/product/bulk_action_progress&code=' . $code . '&page=' . $page . '&list_type=' . $list_type . '&action=' . $action . '&product_list=' . $data['product_list'] . '&bypass=' . $bypass . '&' . $this->token_data['token_link'] . '&total_for_progress=' . $total_for_progress;

        } else if($list_type == 'selected') {

            $url = 'index.php?route=entegrasyon/product/bulk_action_progress&code=' . $code . '&page=' . $page . '&list_type=' . $list_type . '&action=' . $action . '&product_list='.$data['product_list'].'&' . $this->token_data['token_link'] . '&bypass=' . $bypass . '&total_for_progress=' . $total_for_progress;

        }else {
            $url = 'index.php?route=entegrasyon/product/bulk_action_progress&code=' . $code . '&page=' . $page . '&list_type=' . $list_type . '&action=' . $action . '&product_list=&bypass=' . $bypass . '&' . $this->token_data['token_link'] . '&total_for_progress=' . $total_for_progress;
